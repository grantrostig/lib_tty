...
CXXFLAGS = $(if $(findstring 0,$(CXXOPT)),-U_FORTIFY_SOURCE,-D_FORTIFY_SOURCE) \
	-std=$(CXXSTD) -O$(CXXOPT) -ggdb$(CXXDBG) \
	$(if $(findstring 0,$(RELAXED_WARNS)),-Werror) \
	-pedantic \
	-Wall \
	-Wcast-align \
	-Wconversion \
...
CURR_OPTS = $(filter-out run .DEFAULT debug all diagnostic,$(MAKECMDGOALS)) \
	$(CXX) -std=$(CXXSTD) -O$(CXXOPT) -ggdb$(CXXDBG) $(STRIP_BINS) $(RELAXED_WARNS)
