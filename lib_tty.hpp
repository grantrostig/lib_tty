...
/** The user level intent of a pressed HotKey of during HotKeyFunctionCat::nav_intra_field  // TODO: rename to make consistent, re: nav, intra...
 */
...
*/
using Key_char_singular = KbFundamentalUnit;                // The most basic character that a keyboard can generate like ASCII or UNICODE 8? or
                                                            //  16? or 32?, which does not generate a multi-btye burst of characters, like F1
using Hot_key_chars 	= std::vector< Key_char_singular >; // A sequence of basic chars that are generated by a user single keypress of a
                                                            //  Hot_key on a keyboard, ie. ESC, or F1 for help. but does not include a or 2 or
                                                            //  '/', but NOTE they could also enter them manually char by char.
using I18n_key_chars 	= std::vector< Key_char_singular >; // One or a sequence of singular chars that are entered or generated by a user using
                                                            //  ASCII, Extended ASCII, or other international KB characters.
 ...
 *  TODO: An international i18n char could look like a hotkey since it might start with <ESC> or something else special,
